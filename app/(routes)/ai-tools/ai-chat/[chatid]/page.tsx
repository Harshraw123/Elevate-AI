...
const Aichat: React.FC<AichatProps> = ({ selectedQuery }) => {
  ...
  const sendData = async () => {
    ...
    try {
      ...
      if (response.data.output) {
        const cleanOutput = response.data.output; // You can sanitize here
  ...
  return (
    <div className="flex flex-col h-screen p-4 space-y-4 bg-black text-white">
      {/* Header */}
      ...
      {/* Chat Area */}
      <div className="flex flex-col flex-1 border rounded-md p-4 overflow-hidden min-h-screen">
        <div className="flex-1 overflow-y-auto">
          {messages.length === 0 ? (
            ...
            <div className="space-y-4">
                    ...
                    <ReactMarkdown
                      ...
                      components={{
                        ...
                        code: ({ inline, children }) =>
                          inline ? (
                            <code className=" text-pink-400 px-1 py-0.5 rounded text-sm">{children}</code>
                          ) : (
              ...
              {loading && (
                <div className="p-4 rounded-lg max-w-[80%] mr-auto flex items-center gap-2">
                  <Loader />
                </div>
              )}
          ...
          {error && (
            <div className="text-red-500 mt-4 p-4 bg-red-100/10 rounded-md">
              {error}
            </div>
          )}
        </div>
